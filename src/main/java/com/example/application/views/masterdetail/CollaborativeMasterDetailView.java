...
@Route(value = "collaborative-master-detail/:samplePersonID?/:action?(edit)", layout = MainView.class)
@PageTitle("Collababorative Master-Detail")
public class CollaborativeMasterDetailView extends Div
    ...
    private CollaborationList notifications;
    ...
    public CollaborativeMasterDetailView(
        ...
        TemplateRenderer<SamplePerson> importantRenderer = TemplateRenderer
                .<SamplePerson> of(
        ...
        save.addClickListener(e -> {
            try {
                ...
                notifications.append(this.samplePerson);
                UI.getCurrent().navigate(CollaborativeMasterDetailView.class);
        ...
        CollaborationEngine.getInstance().openTopicConnection(this,
                "collaborative-mester-detail-notifier", userInfo,
                topicConnection -> {
                    notifications = topicConnection
                            .getNamedList("notifications");
                    notifications.subscribe(change -> {
                        change.getAddedItem(SamplePerson.class)
                                .ifPresent(this::showNotification);
                    });
                    return null;
                });
    }

    private void showNotification(SamplePerson person) {
        Notification.show("Person '" + person.getFirstName()
                + person.getLastName() + "was updated");
    }

    @Override
    public void beforeEnter(BeforeEnterEvent event) {
        Optional<Integer> samplePersonId = event.getRouteParameters()
                .getInteger(SAMPLEPERSON_ID);
    ...
    private void populateForm(SamplePerson value) {
        ...
        String topic = null;
        if (samplePerson != null && samplePerson.getId() != null) {
            topic = "sampleperson/" + value.getId();
